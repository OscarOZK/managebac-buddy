...
enum PreviewFlags {
    ...
    /// ★ 首启分流的真机自检 ★
    /// 值 = "full"（重走完整流程）或 "intro"（只重演开幕）；nil = 什么都不做。
    ///
    /// 为什么非得开这个后门：设置页那两个「重走一遍」按钮要走完
    /// 「点击 → flowRestart → MBApp 换幕 → 换掉子树 identity」整条链，
    /// 而无头环境注入不了点击事件（和上面 autoDrag 同一个理由）。
    /// 走这个开关，启动后自动投下同一枚信号 —— 下游走的是**一模一样**的那条路，
    /// 于是「按钮点了没反应」这种只能在真机上撞见的毛病，能提前被逮住。
    ///
    /// 正常运行时它是 nil：入口只有一个环境变量（MBB_AUTO_RESTART），
    /// 只在命令行手跑自检时设，界面上没有任何入口能写它。
    static var autoRestart: String? = nil
}
